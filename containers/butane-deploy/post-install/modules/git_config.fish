...
function git_config
    info_print "\nConfiguration: Git Name and Email"
    if read_confirm
        ...
        command_print "git config --global user.email $GIT_EMAIL"
        if git config --global user.email $GIT_EMAIL
            success_print "Successfully set Git email to $GIT_EMAIL!"
        else
