function install_determinate_nix
    ...
    if gum confirm "Install the Nix Package Manager using the Determinate Nix Installer?"
        if gum spin --title "Installing Nix" --show-output -- curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install linux --determinate --init none
